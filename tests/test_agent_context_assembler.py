...
async def test_assemble_system_head_memory_and_plan_tail():
    ...
    msgs = await ParkContextAssembler().assemble(_cfg("main"), conv)
...
async def test_assemble_wraps_tool_results_two_tiers():
    conv = Conversation(thread_id="t")
    ...
    msgs = await ParkContextAssembler(keep_recent_turns=10).assemble(_cfg(), conv)
...
async def test_assemble_valid_role_sequence_no_orphan():
    conv = Conversation(thread_id="t")
    conv.append(Message(role="user", content="hi"))
    ...
    msgs = await ParkContextAssembler().assemble(_cfg(), conv)
    assert repair_messages(list(msgs)) == 0          # 无悬空 tool_call、无孤儿 result


async def test_soft_token_cap_warns_and_feeds_awareness_no_drop(caplog):
    ...
    with caplog.at_level(logging.WARNING):
        msgs = await asm.assemble(_cfg(), conv)
...
async def test_no_awareness_note_when_under_cap():
    """未超阈 → 不注余量提示(有空间时不打扰)。"""
    conv = Conversation(thread_id="t")
    conv.append(Message(role="user", content="hi"))
    msgs = await ParkContextAssembler(soft_token_cap=100000).assemble(_cfg(), conv)
    assert not any("上下文余量" in (m.content or "") for m in msgs)


async def test_control_result_framed_as_executed_not_state():
    ...
    msgs = await ParkContextAssembler(control_tools=frozenset({"device_ctrl"})).assemble(_cfg(), conv)
...
async def test_assemble_uses_compaction_summary():
...
async def test_compaction_with_plan_calls_valid_view():
    ...
    pair = await ConversationCompactor(FakeSummarizer("【摘要】中段已压"),
...
async def test_subagent_result_framed_as_report_not_state():
    ...
    msgs = await ParkContextAssembler(subagent_tools=frozenset({"device_agent"})).assemble(_cfg(), conv)
