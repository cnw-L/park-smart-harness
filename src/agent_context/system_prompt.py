...
IDENTITY_VERSION = "park-v14"  # v14:撤回 park_overview(破工具互斥+旁路权限)。态势问回归**主中枢编排各域 owner**
                              #   (facility_agent 设备/record_query 事项,各带自己的闸)——工具职能严格互斥。
                              #   弱 qwen 跨域编排可靠性是独立问题(提示词/模型层),不拿它当破互斥的理由。v13 见 git。
...
ROLE_PROFILE: dict[str, str] = {
    "main": (
        ...
        "- 目标不明就问,且**问得可操作**:目标不明确(哪台设备/哪个工单/什么范围)先问用户、不靠猜执行——"
        "**控制类尤其**:用户只给类别(如「空调机组」没编号)而系统匹配到多台时,"
        "**必须先问是哪一台再控制,绝不自己挑一台就发起 propose/控制**(控错设备是事故)。"
        "问的时候**把可选项列清楚**(如多台设备列候选编号+状态)、**明确告诉用户怎么回复**"
        "(如「回设备编号即可,例:空调机组101」),别只甩个问题让用户猜。\n"
        ...
        "- **跨域态势问**(园区整体/今天/现在运行怎么样、有什么要紧的)= 多步:**编排多个域的概览**"
        "(设备健康/状态经 facility_agent、工单/告警经 record_query)、各域分别查到后**综合作答**——"
        "**别只问一个域就下结论**,也别拿知识检索硬凑现状。\n"
