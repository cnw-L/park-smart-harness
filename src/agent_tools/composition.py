...
# ★工具职能严格互斥(否则职能不清、难治理):每个顶层工具/子 agent 拥有不重叠的主题域。
# 跨域态势("今天园区怎么样")**不设跨域工具**(会破互斥+旁路权限)——由**主中枢编排**各域 owner
# (facility_agent 设备 / record_query 事项,各带自己的权限闸)综合,这正是 V6"中枢编排"的职责。
...
class ToolSubsystem:
    ...
    verifier: object = None              # 注入 run_loop(verifier=...);控制结果业务校验(已受理≠已生效)
...
def build_tool_subsystem(*, model_caller: ModelCaller, backend: BackendClient | None = None,
                         ...
                         assembler=None, execution_mode: str = "simulated",
                         idem_ledger=None, store=None) -> ToolSubsystem:
    # 单例 —— 子的 propose_control 与父侧 control 共享。注入 RedisProposalStore 即多实例/跨重启共享;
    # 默认进程内 ProposalStore(单实例 demo/单测)。
    store = store if store is not None else ProposalStore()
    ...
    # 子 agent 聚合多次模型调用(最坏 5×) → 给宽超时,执行器到点掐断防 runaway(见 dispatch wait_for)
    facility_tool = replace(facility_tool, timeout_s=120.0)
    ...
    # 知识检索内部 RAG provider 超时 60s → 执行器给 75s(必须 >60,否则误杀正常检索)
    catalog.register(ToolSpec(tool=replace(make_knowledge_query_tool(retriever), timeout_s=75.0),
                              capability_code="knowledge:read", output_budget=1500))
    ...
    return ToolSubsystem(
        ...
        gate=gate,
        store=store,
        verifier=ControlVerifier(),         # 替 NullVerifier 桩:控制 effective=False/pending → verify-failed
    )
